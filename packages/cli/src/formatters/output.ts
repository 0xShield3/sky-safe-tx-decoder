 ...
 */
function verificationBadge(verification: { verified: boolean; status?: DecodeVerificationStatus }): string {
...
export function printNestedTransactionData(
  ...
  console.log(
    `${chalk.dim('Operation')}: ${chalk.green(operation === 0 ? 'Call' : operation === 1 ? 'DelegateCall' : 'Unknown')}`
  );
  ...
  // Neither a custom decoder nor the Safe API could decode this nested call.
  // Show the function selector and the FULL calldata — never a truncated
...
function printDecodedFunction(func: DecodedFunction, index: number): void {
  ...
  if (func.riskLevel && func.riskLevel !== 'none') {
    const riskColor = func.riskLevel === 'high' ? chalk.red : func.riskLevel === 'medium' ? chalk.yellow : chalk.blue;
